...
const register = async (req, res) => {
  try {
    ...
    const newUser = await pool.query(
      ...
      RETURNING id, name, email, role, organization_id, team_id, agent_token`,
      [
        name,
        email,
        hashedPassword,
...
const login = async (req, res) => {
  try {
    ...
    const user = await pool.query(
      "SELECT id, name, email, password, role,organization_id,team_id  FROM users WHERE email = $1",
      [email]
    );
