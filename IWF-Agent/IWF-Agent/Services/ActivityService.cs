...
if (!string.IsNullOrEmpty(restrictedName))
{
    ...
    {
        // First tick where this restricted app/site is seen.
        // Fire the alert immediately instead of waiting for a
        // dwell-time threshold. The backend's own 30-minute
        // duplicate-suppression window (per employee+website)
        // still prevents repeat emails while it stays open.
        ...
        Console.WriteLine(
            $"INSTANT ALERT => Site: {restrictedName}, UserId: {UserContext.UserId}"
        );
